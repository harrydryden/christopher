...
export default async function ApplicationsPage() {
  ...
  return <div className="max-w-4xl space-y-5"><PageHeader title="Applications" />
    ...
    {rows.map(row => <section key={row.id} className="space-y-3 rounded border p-4">
      ...
      <div className="flex gap-4 text-sm"><a className="underline" href={`/api/applications/${row.id}/pdf`}>Download submitted CV</a>{row.cvId && <Link className="underline" href={`/cv/${row.cvId}`}>View submitted revision</Link>}</div>
      <SettingsForm action={updateApplication.bind(null, row.id)} submitLabel="Save application update">
        <label>Status<select name="status" defaultValue={row.status} className="ml-2 rounded border p-2">{["applied", "screening", "interview", "offer", "rejected", "withdrawn", "accepted"].map(s => <option key={s}>{s}</option>)}</select></label>
        <label>Notes<textarea name="notes" defaultValue={row.notes} maxLength={4000} className="block w-full rounded border p-2" /></label>
      </SettingsForm>
