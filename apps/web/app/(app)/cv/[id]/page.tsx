  ...
  return <div className="max-w-4xl space-y-5">
    ...
    {content && <>
      ...
      <SettingsForm action={saveCvDraft.bind(null, id)} submitLabel="Save as new revision">
        ...
        {content.sections.map((section, i) => <label key={section.entryId} className="text-sm"><span className="font-semibold">{section.heading}</span>{!!section.industryDescriptions?.length && <span className="mt-1 block text-xs text-slate-500">{section.industryDescriptions.join(" · ")}</span>}<textarea name={`section-${i}`} defaultValue={section.bullets.join("\n")} rows={Math.max(3, section.bullets.length * 2)} className="mt-1 block w-full rounded border p-2 dark:bg-slate-950" /><span className="text-xs text-slate-500">One bullet per line.</span></label>)}
