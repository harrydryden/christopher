...
export default async function CvPage({ searchParams }: { searchParams: Promise<{ job?: string; q?: string; page?: string; archivedPage?: string }> }) {
  ...
  return <div className="max-w-6xl space-y-6">
    ...
    <CvManagement key={`${saved.page}:${archived.page}`} savedPage={saved.page} archivedPage={archived.page}><Card title="Saved CVs"><p className="mb-3 text-14 text-muted">Each company and role keeps one saved CV and one archived predecessor. A build that failed stays here until you start the next revision for that role, which replaces it; a revision that builds successfully clears the failed attempts. Builds still queued or in progress are never removed.</p><CvSavedTable key={`saved-${saved.page}`} rows={drafts} /><CvPagination page={saved.page} total={saved.total} path="/cv" params={{ ...params, archivedPage: String(archived.page) }} label="Saved CV pages" /></Card>
