...
function SuggestionCard({ row, returnTo = "/suggestions" }: { row: SuggestionRow; returnTo?: string }) {
  ...
  return <article aria-label={suggestion.name} className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
    ...
    {suggestion.evidence && <section className="rounded bg-slate-50 p-3 text-sm dark:bg-slate-900">
...
export default async function SuggestionsPage({ searchParams }: { searchParams: Promise<{ view?: string; notice?: string; page?: string; q?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    {view === "sources" ? <DiscoverySources/> : view === "history" ? <>
      ...
      {!q && !pending.length && !active.length && <div className="mt-3 text-center"><a href="/suggestions?view=sources" className="inline-block rounded bg-slate-900 px-4 py-2 text-sm font-semibold text-white dark:bg-slate-100 dark:text-slate-950">Add a discovery source</a></div>}
    </>}
  </div>;
}
