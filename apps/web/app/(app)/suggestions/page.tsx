...
function SuggestionCard({ row, returnTo = "/suggestions" }: { row: SuggestionRow; returnTo?: string }) {
  ...
  return <article aria-label={suggestion.name} className="space-y-3 rounded-lg border border-slate-200 p-4">
    ...
    <div className="flex flex-wrap items-center gap-2 text-sm">
      ...
      {verification?.careersSource && <a href={verification.careersSource.url} target="_blank" rel="noreferrer" className="text-accent underline">View careers page ↗</a>}
    </div>
    ...
    {suggestion.evidence && <section className="rounded bg-slate-50 p-3 text-sm">
      ...
      <div className="mt-2">{suggestion.evidence.url ? <a href={suggestion.evidence.url} target="_blank" rel="noreferrer" className="text-accent underline">{suggestion.evidence.title} ↗</a> : <p>{suggestion.evidence.title}</p>}
...
export default async function SuggestionsPage({ searchParams }: { searchParams: Promise<{ view?: string; notice?: string; page?: string; q?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    {view === "sources" ? <DiscoverySources/> : view === "history" ? <>
      ...
      {!q && !pending.length && !active.length && <div className="mt-3 text-center"><a href="/suggestions?view=sources" className="inline-block rounded bg-accent px-4 py-2 text-sm font-semibold text-white">Add a discovery source</a></div>}
    </>}
  </div>;
}
