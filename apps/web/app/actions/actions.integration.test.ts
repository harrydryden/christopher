...
describe("priority workflows", () => {
  ...
  it("versions libraries and snapshots generation inputs atomically with its task", async () => {
    ...
    expect(draft!.librarySnapshot).toEqual({ ...content, structuredExperience: true, entries: [{ ...content.entries[0], heading: "Director · Acme · Aug 2023 – Present" }] }); expect(draft!.model).toBe("claude-fable-5-1");
