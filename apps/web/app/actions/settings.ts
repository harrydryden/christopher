...
export async function saveKeywords(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await setSetting("gate", { ...settings.gate, includeKeywords, excludeKeywords });
...
export async function saveMatchFields(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await setSetting("gate", { ...settings.gate, matchFields: matchFields.length ? matchFields : ["title"] });
  await enqueue("reevaluate_gate", {});
  revalidatePath("/settings");
  revalidatePath("/");
  return ok();
...
export async function saveLocationFilter(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await setSetting("gate", { ...settings.gate, locationTerms, includeRemote });
  await enqueue("reevaluate_gate", {});
  revalidatePath("/settings");
  revalidatePath("/");
  return ok();
...
export async function saveTableSettings(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  await setSetting("nearMissMinScore", nearMissMinScore);
  revalidatePath("/settings");
  revalidatePath("/");
  return ok();
}
...
export async function saveSchedule(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  if (!Number.isInteger(closeAfterMissingScans) || closeAfterMissingScans < 1 || closeAfterMissingScans > 5) {
    return fail("Close-after-missing-scans must be a whole number between 1 and 5.");
  }
