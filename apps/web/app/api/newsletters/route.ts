...
import { isImportOnlyKind, sha1, stripHtml } from "@christopher/core";
...
export async function POST(request: Request): Promise<Response> {
  ...
  } catch { return Response.json({ error: "Expected sourceId or to, a title or subject, and content of at least 100 characters" }, { status: 400 }); }
  ...
  if (!sourceId) return Response.json({ error: "No source is subscribed at that address" }, { status: 404 });
  ...
  if (!source || !isImportOnlyKind(source.kind)) return Response.json({ error: "Email source not found" }, { status: 404 });
