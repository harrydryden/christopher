...
export function CvAppearance({ value, onChange, name }: { value?: CvTheme; onChange?: (theme: CvTheme) => void; name?: string }) {
  ...
  return <fieldset className="space-y-4 rounded-lg border border-slate-300 p-4">
    ...
    <div aria-label="Palette sample" className="space-y-3 rounded p-4" style={{ background: theme.background, color: cvForeground(theme.background) }}>
      ...
      <div className="flex flex-wrap gap-2">{['Financial planning', 'Team leadership', 'SQL'].map(skill => <span key={skill} className={theme.skillPills ? 'rounded-full px-3 py-1 text-sm' : 'text-sm'} style={{ background: theme.skillPills ? theme.pill : theme.background, color: cvForeground(theme.skillPills ? theme.pill : theme.background) }}>{skill}</span>)}</div>
    </div>
  </fieldset>;
}
