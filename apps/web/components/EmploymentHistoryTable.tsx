...
export function EmploymentHistoryTable({ employment, entries, onChange }: {
  ...
  return <section aria-labelledby="employment-heading" className="space-y-3 rounded border border-slate-200 p-3">
    ...
    <p className="text-sm text-slate-500">One row per job. Select this job on each related experience block below. Company, title and dates are maintained here and used for future CVs.</p>
    ...
    <div className="overflow-x-auto"><table className="w-full text-left text-sm">
      <thead><tr>{["Company", "Job title", "Start date", "End date", "Current", "Evidence", ""].map((label, i) => <th className="p-2" key={i}>{label}</th>)}</tr></thead>
      <tbody>{employment.map((job, i) => {
        const count = entries.filter(entry => entry.employmentId === job.id).length;
        return <tr key={job.id}>
          ...
          <td className="p-2">{count} {count === 1 ? "block" : "blocks"}</td>
          <td className="p-2"><button type="button" disabled={count > 0} title={count ? "Reassign or remove linked evidence first" : "Remove job"} aria-label={`Remove job ${i + 1}`} className="underline disabled:opacity-40" onClick={() => onChange(employment.filter(item => item.id !== job.id))}>Remove</button></td>
        </tr>;
      })}</tbody>
    </table></div>
    {!employment.length && <p className="text-sm">Add your first job, then link experience evidence to it.</p>}
