...
export function Pagination({ page, total, size = 50, path, params = {} }: {
  page: number; total: number; size?: number; path: string; params?: Record<string, string>;
}) {
  const pages = Math.max(1, Math.ceil(total / size));
  const href = (n: number) => `${path}?${new URLSearchParams({ ...params, page: String(n) })}`;
  return <nav aria-label="Pagination" className="my-4 flex flex-wrap items-center justify-between gap-3 text-sm">
