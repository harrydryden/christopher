...
export function WorkspaceNav() {
  ...
  return <nav aria-label="Workspace sections" className="ds-divider mb-5 flex flex-wrap gap-4 pb-3 text-13">{group.map(([href, label]) => <Link prefetch={false} key={href} href={href!} aria-current={path === href ? "page" : undefined} className={path === href ? "font-semibold text-fg underline decoration-2 underline-offset-4" : "text-muted no-underline hover:underline"}>{label}</Link>)}</nav>;
}
