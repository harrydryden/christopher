...
import { setRowFacet, type CvLibrary, type Employment } from "@christopher/core/cv";
import {
...
function library(rows: Record<string, string[]>, facets: Record<string, Record<string, string>> = {}): CvLibrary {
  const entries = Object.entries(rows).map(([id, lines]) => {
    ...
    for (const [row, facet] of Object.entries(facets[id] ?? {})) entry = setRowFacet(entry, row, facet as never);
    return entry;
  });
  return {
...
const view = (over: Partial<EvidenceEntryView> = {}): EvidenceEntryView => ({
  ...
  source: "model", provisional: false, evaluating: false, missing: [], missingLine: "All six facets are covered",
  prompts: [], reviewedRows: [], ...over,
});

it("names the two facets worth the most and counts the rest", () => {
  expect(missingFacetLine([])).toBe("All six facets are covered");
  expect(missingFacetLine(["metric"])).toBe("No metric yet");
  expect(missingFacetLine(["outcome", "metric"])).toBe("No outcome or metric yet");
  expect(missingFacetLine(["outcome", "metric", "milestone"])).toBe("No outcome or metric yet · 1 other facet untagged");
  expect(missingFacetLine(["outcome", "metric", "milestone", "style"])).toBe("No outcome or metric yet · 2 other facets untagged");
});

it("reads the facets a job is not tagged with, the heaviest first", () => {
...
it("falls back to the baseline for an entry nobody has reviewed in this wording, and says so", () => {
  ...
  expect(entry.missingLine).toBe("No outcome or problem solved yet · 2 other facets untagged");
  // Each prompt is one question, and the baseline's questions say which facet they are about.
...
it("shows a stored model review as it was computed and stops saying Evaluating", () => {
  ...
  const review: LibraryEntryReview = {
    entryId: "acme",
    rows: [
      { row: "Led a team", facet: "responsibility", specific: false, quantified: false, outcomeLinked: false, quote: "Led a team", verified: true },
      { row: "Cut handovers by 40%", facet: "metric", specific: true, quantified: true, outcomeLinked: true, quote: "Cut handovers by 40%", verified: true },
    ],
  ...
  // A question the model wrote itself is left untagged rather than guessed at.
