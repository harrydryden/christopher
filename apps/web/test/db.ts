 ...
 * the interface.
 *
 * `lib/db.ts` opens three connections per serverless instance. A suite on a larger pool passes
 * where production deadlocks: a transaction that asks the pool for a second connection finds one
 * spare here and waits out the connection timeout there. So every suite gets the same three, and a
 * test that hangs on them has found a request that needs more than production has.
 */
