...
The strengthened CV evaluation ran its preflight and correctly exited unsuccessfully with `blocked`, zero spend and `releaseAccepted: false` because no `ANTHROPIC_API_KEY` was available at that stage. The subsequent authorised live run and grounding fix are documented in [CV provider evaluation](CV-PROVIDER-EVALUATION.md#executed-live-evaluation--20-september-2026). The [report](benchmarks/provider-evaluation-2026-09-20.json) is evidence of a blocker, not generation success. [The evaluator instructions](CV-PROVIDER-EVALUATION.md) explain the conservative cost guardrail, provider-price provenance and required manual PDF review.
...
| Provider/CV quality | Extend representative quality coverage and verify hosted generation/configuration before release. | The default CV model passed a real five-case semantic evaluation and one-page generation after fixing unsupported writer embellishment. Astra/Sol textual and visual checks passed; total estimated token cost $0.927517. This narrow synthetic result does not certify broader or hosted behaviour; see [evaluation](CV-PROVIDER-EVALUATION.md). |
...
## Final-check development follow-up

The provider key can access the production global default (`claude-sonnet-5`) and repository CV default (`claude-fable-5-1`). This was a free model-metadata preflight, with zero generation calls; per-account production model overrides and hosted generation were not checked. See [model access evidence](benchmarks/provider-model-access-2026-09-20.json).

Operational health now includes persisted crash-recovery history, provider failures grouped by call site/model, aggregate spend and account-budget attention. Two crash recoveries in an hour fail the gate; three failures with no success for one provider group are a documented heuristic. Budget exhaustion warns without declaring an outage, and deliberate zero-budget/no-spend accounts remain quiet. Database-backed tests cover cancellation, null errors, unrelated successful groups, reset boundaries and budget defaults. Notification delivery still requires external proof.

Migration execution now refuses known Render transaction-pooled connection URLs before taking a session advisory lock. Worker boot and migrations require the direct endpoint; the web connection may use the pooled endpoint. The [hosting report](HOSTED-CAPACITY-2026-09-20.md) records the documented pool defaults separately from unverified live endpoint selection.

Browser fallback now shares the HTTP fetcher's robots policy. Chromium interception checks each main-frame navigation before transmission, including server and JavaScript redirects; service workers are blocked. Real HTTP regressions prove a denied redirect destination receives no requests, and late navigation errors cannot be returned as successful pages. A permitted 37signals render completed and retained its explicit empty state. The broader source/posting accuracy gate remains open.

The controlled worker run establishes a concrete sizing concern: nine jobs completed at 512 MiB, but 466.8 MiB peak cgroup memory failed the 358.4 MiB headroom threshold. A matching 1 GiB local run passed at 411.1 MiB. No hosting plan was changed. Full methods, retained earlier observations and limitations are in [capacity/recovery evidence](CAPACITY-AND-RECOVERY-DRILLS.md).

The restored synthetic application passed five authenticated GET endpoints after inserting and then removing a dedicated short-lived test session. The original expired-session failure is retained. This check also exposed an API bug: a valid signed cookie with an expired/revoked database session could produce HTTP 500. Affected protected API routes now return private, non-cacheable HTTP 401 responses without changing server-action authentication. Unexpected database and programming errors remain server failures. Managed-copy application recovery and executed runtime rollback remain separate open gates.

The final built restored-app journey also reused the signed cookie after deleting its database session. The status endpoint correctly returned HTTP 401 with `private, no-store`; no temporary session rows remained. Regression fixtures that intended immediate retry eligibility now use the database clock, removing a roughly one-millisecond host/database clock dependency without altering production backoff or ownership rules.

### Final verification of this follow-up

On Node 22.18.0, all **281 worker tests**, **494 web tests** and **33 release-script tests** passed (808 tests). Worker and web database suites ran against separate dedicated local PostgreSQL 16 databases. All package type checks and the final production web build passed. The worker total includes seven enabled Chromium tests. Earlier diagnostic runs exposed the redirect-error bug, clock-sensitive fixtures and interference from concurrent suites sharing a test database; the final isolated runs passed after those causes were addressed. The core/AI suites were not rerun in this final follow-up because their implementation was unchanged; their earlier passing evidence remains separate.

No paid AI generation, hosted plan change, live candidate deployment, push, PR or merge was performed during this follow-up. The retained managed recovery copy remains provisioned under the earlier approval. The local changes are ready for review, but the release gates in the table above are still open.
