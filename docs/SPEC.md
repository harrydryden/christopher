...
- **One scan a day per company.** The daily run scans each company with at least one active follower exactly once and evaluates every follower's gate against what it observes. A manual rescan reuses a scan made in the last 30 minutes. The schedule, models, the shared monthly AI budget and robots policy are system settings an administrator sets; Health shows AI spend per account.
- **AI budgets are per account as well as shared.** Every account has its own monthly budget, $25 to start, which an administrator raises per account in Admin › Accounts; the shared `monthlyAiBudgetUsd` remains the ceiling on everything, including work done for no particular account (extraction, discovery). Both are the sum of the `ai_calls` rows since the window began, so nothing has to keep a running total straight: the window starts at the beginning of the UTC month, or at a recorded reset (`aiBudgetResetAt`, per account and shared) when that is later. Resetting a counter therefore moves the window and never deletes the call log.
...
- Use Anthropic with a separately editable `cvModel`, default `claude-fable-5-1`, chosen from the supported model list and distinct from scraping call site A3. Generation uses the worker's `ANTHROPIC_API_KEY`, the account's monthly AI budget and the shared ceiling. Model configuration is captured per draft. Repeated queue delivery must not regenerate a completed draft.
...
- A build is admitted against the AI budget once, up front, at what it is expected to cost (calibrated on recorded builds; about $3 for a 35 KB library on Fable 5.1), and refused before it spends anything with the limit, the amount left and the amount held by work in flight named in the draft's error. It is admitted twice over: first against the account's own monthly budget, whose refusal names that budget, what is left of it this month and that an administrator can raise it in Admin › Accounts; then against the shared ceiling, whose refusal names the shared budget and Admin › System settings. The calls inside an admitted build are not held again, so a build never fails part-way over budget accounting; a month can end at most one build over its budget.
...
- **R-6.11** Every AI call is logged with tokens and cost, against the account it was made for where there is one. Two monthly budgets bound the spend: the account's own (`aiBudgetUsd`, $25 by default, raised per account by an administrator) and the shared ceiling (`monthlyAiBudgetUsd`), which also covers work with no account behind it. A call is held against the account's budget first and the shared one second; either refusal stops the work and names the budget that refused. Exceeding either budget stops non-essential calls (near-miss scoring first, then suggestions) — the shared one for everyone, an account's for that account alone — and the UI says so; work already queued for an exhausted account is skipped and finishes, never failed and retried. Both budgets count from the start of the UTC month or from a later recorded reset, so an administrator can zero a counter without touching the call log; reports read spend per account, call site and model.
...
**Health.** Failed and suspect scans, sources needing confirmation, blocked sources, re-discovery proposals, AI spend this month against the shared budget and per account.

**Settings.** Keywords, location filter, hide threshold, near-miss toggle, daily run time and timezone, seed profile text, the shared monthly AI budget and each account's own, password change.
...
- Every call writes an `ai_calls` row (call site, model, tokens, cache reads, cost, duration, outcome, and the account it was for when there is one). The Health panel shows spend for the current budget window against the shared budget, and per account against each account's own.
...
settings             key (pk), value jsonb, updated_at        -- system only: scanTime, timezone, models, budget, aiBudgetResetAt, robots, registrationOpen
