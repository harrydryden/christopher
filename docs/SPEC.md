...
- Maintain an editable, ordered evidence library: name, contact details (email, phone and location, each its own field, plus a free-text line for anything else), LinkedIn profile URL, bio, experience, education, skills and interests. A CV header prints the contact details on one line, email · phone · location, then the free-text line. A library saved when contact details were one free-text line opens with an unambiguous email address and phone number moved to their fields and everything else left in that line; nothing is dropped. Entries have stable IDs, headings and source evidence. Add/remove/reorder entries. Saving is only ever the person's own act: the Library pins Save and Discard above the fold once there are unsaved changes. Library saves append immutable versions and reject obsolete edits. Each experience row shows its own evidence score (0–100: a quarter each for having a type, being specific, having a number and being tied to an outcome), from the review of the saved library.
...
- **R-7.3** Header strip: four terse facts for the account — "Last scan 2h ago · Following 6 companies · 3 new role matches · 2 new company matches". Last scan is the most recent completed (not failed) scan of a company the account actively follows, or "not yet"; following counts active subscriptions; new role matches is the Matched tab's count; new company matches counts pending suggestions. Each links to where it counts: Health, Companies, the Matched tab, Suggestions. While the shared run is in progress a small "Scanning now" leads the strip.
...
- **R-8.4** Presentation. Up to 10 suggestions per week, ranked by similarity confidence, then number of currently matching roles. Each shows: name, one-liner, "similar to" chips, open roles, matching roles, rationale. Accept adds the company (with its already-discovered source) and runs the first scan. Reject asks for a reason, which feeds the preference profile (for example "no agencies", "not fintech"); a swipe or arrow-key dismissal may file none, in which case the company is excluded from future suggestions but nothing is learned.
- **R-8.5** A suggestion is never shown twice unless it was expired unseen for 90 days.

### 3.9 Health and attention panel

- **R-9.1** Anything that needs you appears here and nowhere else: sources needing confirmation, three consecutive failures, suspect empties, blocked sources, re-discovery proposals, AI budget exceeded, companies with no source. Health is a section of Settings, reached from its section tabs, and the number of items on it is shown on the Settings entry in the sidebar, because an attention item nobody can see is an attention item nobody resolves.
...
| A12 | Library evidence review | Re-score on the Library (offered once saved rows have changed since their last review; never on a save from the editor, never on a keystroke), an accepted document import, or answered gap questions; per account, debounced onto the newest version | Cached: instructions + the whole library (jobs, every entry's rows, the person's own facet tags). Uncached: up to eight entries under review, with their rows verbatim | `{entries: [{entryId, rows: [{row, facets[], specific, quantified, outcomeLinked, quote}], prompts[]}]}` | low | 15k cached + 1.2k a batch / 0.4k an entry |
...
**Routes, per stage.** Each entry's `route` names its model — `cvModel` (the account's CV model), `callSite` (the deployment's model for the call site) or a fixed model id — and its effort. An administrator may override either for any entry with the `stageRoutes` system setting (`{ "<entry id>": { model?, effort? } }`, validated against the supported models and the provider's effort levels; anything else is dropped on read). A model set there applies to that stage for every account, over its CV model choice. The defaults keep every CV stage on the account's CV model at `high`. The estimates price each stage at the model it is routed to: `estimateCvBuildUsd(model, size, parts, routes)` for a whole build, and `estimateStage(entry, sizes, models)` for one stage admitted on its own, priced by the entry's cache layout.

**Provider hygiene.** Every engine in a process shares one stream governor: at most `AI_MAX_STREAMS` (default 24) streams open per model, with interactive work (a person waiting on a CV, a Library import or review) let through ahead of background work (scans, discovery, suggestions). An engine that builds its own client turns the SDK's retries off and retries itself — only a request that never began, only for a throttle, an overload, a server error or a dropped connection, at most twice — and after a 429 or 529 every request not yet sent waits out one shared pause: at least the provider's `retry-after`, at least a base that doubles with each throttle in a row, at most a minute, released with jitter so the queue does not resume in lock-step. The cap and what is open under it are exposed for Health (`aiGovernorStats()`).

**Audit batches end one by one.** `assessCvBatches(input, ref, { pass, only, onBatch })` returns every batch it ran as `done` (with its result), `failed` (with its error and named failure) or `cancelled` (stopped because a sibling failed — no error of its own — or because the run was stopped, which its error then says). A failed batch still stops the siblings in flight, but the batches that had finished are kept, so a caller can checkpoint them and re-run only the lost ones with `only`; the batches are sliced from the input the same way every time, so an index names the same requirements and claims. `mergeCvAssessBatches` joins finished batches into the review. The first batch still goes alone until its response begins. `pass: "revision"` marks the audit of the improved candidate: its batch events carry the pass, and its calls record the stage `review_candidate`. `assessCv` keeps its older contract (the whole review, or null when any batch failed).
...
- Every call writes an `ai_calls` row (call site, model, tokens, cache reads, cost, duration, outcome, the account it was for when there is one, and — for a feature that makes several calls — which step of it this was), and since migration 0040 the prompt that produced it (`prompt_id`, `prompt_version`), how its stream behaved (`ttft_ms` from sending to the first event, `max_event_gap_ms` the longest silence between events), the provider's `stop_reason` and `request_id`, how many times the request was sent (`attempt`), and the caller's step (`step_id`). Each is per call, so a percentile of latency is taken over rows. The write checks for the columns first, so a release ahead of the migration still records the call and its cost. Operations shows total spend for the month with usage by account, feature and model, the same lines again with latency and cache hit rate, and the last twenty CV builds itemised by step; Accounts shows each account against its own budget and window.
- **Stages.** A multi-call feature names its steps, so its bill can be explained rather than only summed. A CV build writes `rubric`, `planning` (the evidence plan written before the CV), `author`, `improvement` (the one optional rewrite with the audit's evidence-backed findings), `review` (the audit of the written CV), `review_candidate` (the re-audit of that improved candidate, recorded apart so the two audits' costs never merge), and `_retry` after either audit stage for the re-run of a batch whose source attribution had to be corrected. An evidence review (A12) writes `review` per batch, and `review_retry` for the one re-ask of a batch that left entries uncovered. A single-call feature records no stage.
- **The outcome taxonomy.** `ok` is a call that returned a valid result. `cancelled` is a batch this engine stopped paying for because a sibling in the same task had already failed — a cost deliberately cut short, not a fault. `stalled` is a stream this engine cut off because no event had arrived for the idle timeout (five minutes by default, `AI_STREAM_IDLE_MS`; adaptive thinking can stream nothing for a while) or because it was still open at the fifteen-minute ceiling: a vendor symptom. The error says which ("no events for 5 minutes", "still open after 15 minutes"), and the named failure carries it as `stall`. `failed` is everything else, and is the only one an operator should chase. The taxonomy is derived from the recorded `ok` and error text rather than stored, so no column carries it and rows written before it existed classify the same way. Counting a cancellation as a failure turns one bad build into four broken calls, which is exactly the figure someone would act on.
