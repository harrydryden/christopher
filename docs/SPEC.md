...
- Use Anthropic with a separately editable `cvModel`, default `claude-fable-5-1`, distinct from scraping call site A3. Generation uses the worker's `ANTHROPIC_API_KEY` and monthly AI budget. Model configuration is captured per draft. Repeated queue delivery must not regenerate a completed draft.
