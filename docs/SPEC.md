...
| A12 | Library evidence review | A Library save, per account, debounced onto the newest version | Cached: instructions + the whole library (jobs, every entry's rows, the person's own facet tags). Uncached: up to eight entries under review, with their rows verbatim | `{entries: [{entryId, rows: [{row, facet, specific, quantified, outcomeLinked, quote}], prompts[]}]}` | low | 15k cached + 1.2k a batch / 0.4k an entry |
...
- **The Library says whether a CV can be built from it, by the rule generation applies.** Each job carries what it has and what it still needs — "3 of 5 rows confirmed · draft", with one control that confirms every row of that job — and the page carries one line for the whole Library: "Ready to build: yes", or "Ready to build: no" naming the nearest block and the one thing that would make it usable ("activate Acme and confirm its rows"). Both are computed with `eligibleCvEvidence` and the active-block rule `groupCvLibrary` refuses on, so the Library and the build can never disagree, and neither gates anything: they say what is missing, the person decides.
