...
- Cron schedules work but does not execute long-running jobs by default. The Render worker consumes the queue. `AVA_SERVERLESS_FALLBACK=1` explicitly enables the legacy bounded fallback, only without a fresh worker heartbeat; its individual tasks can still exceed platform limits, so it is not the production recommendation.
