...
it("splits an audit into batches of at most the given size, naming each claim's required source", () => {
  const requirements = Array.from({ length: 19 }, (_, i) => ({ ...rubric.requirements[0]!, id: `r${i}` }));
  const claims = Array.from({ length: 5 }, (_, i) => ({ id: `c${i}`, text: "Led operations", requiredEvidenceId: i ? "entry:role:1" : undefined }));
  ...
  expect(batches.map(batch => [batch.requirements.length, batch.claims.length])).toEqual([[8, 5], [8, 0], [3, 0]]);
  expect(batches[0]!.claimSources).toEqual([evidence[1]]);
  expect(batches[1]!.claimSources).toEqual([]);
