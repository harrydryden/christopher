...
const { expected, url } = requiredReleaseConfig(process.env, "WEB_HEALTH_URL");
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (health.healthy && health.commit === expected) {
        console.log(`Web is healthy and running merged commit ${expected}.`);
        process.exit(0);
      }
    } else reported = `HTTP ${response.status}`;
  } catch { reported = "health endpoint unavailable"; }
  if (attempt % 6 === 0) console.log(`Waiting for web release ${expected}; currently ${reported}.`);
  attempt += 1;
  const remaining = deadline - Date.now();
  if (remaining > 0) await sleep(Math.min(10_000, remaining));
}
throw new Error(`The web application has not deployed merged commit ${expected}. Running release: ${reported}.`);
